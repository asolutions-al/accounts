...
export const sessions = pgTable(
  ...
  (table) => {
    return {
      sessionToken_key: uniqueIndex("sessions_sessionToken_key").using(
        "btree",
        table.sessionToken
      ),
...
export const users = pgTable(
  ...
  (table) => {
    return {
      ...
      stripe_customer_id_key: uniqueIndex("users_stripe_customer_id_key").using(
        "btree",
        table.stripe_customer_id
      ),
      stripe_subscription_id_key: uniqueIndex(
        "users_stripe_subscription_id_key"
      ).using("btree", table.stripe_subscription_id),
    }
  }
)
...
export const verification_tokens = pgTable(
  ...
  (table) => {
    return {
      ...
      token_key: uniqueIndex("verification_tokens_token_key").using(
        "btree",
        table.token
      ),
    }
